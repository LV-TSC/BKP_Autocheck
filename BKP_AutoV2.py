...
def check_lastbackup(FOLDER):
    list_of_files = glob.iglob(FOLDER)
...
def sqlvalidation(server,idsql,date,datereg):
    if check_lastbackup(server) == True:
        crearconexion()
        try:
            with conexion.cursor() as cursor:
                consulta = "use SRV_BACKUP update TBL_RECORDS set idstatus = '3', reg_user = 'RPA', reg_date = ? where idserver = ? and bkp_date = ?"
                cursor.execute(consulta, datereg, idsql, date)
                conexion.commit()
        except Exception as e:
            print(e)
        finally:
