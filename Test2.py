...
ID30_SERVICIOS_LIMA = (r"\\172.16.88.10\bk_v\BK_SERVICIOS_LIMA\172.16.47.5\*")
...
if __name__ == "__main__":
    try:
        ...
        for x in os.listdir(deletefiles):
            if os.path.isfile(x):
                if not(x.endswith(extensions)):
                    os.remove(str(deletefiles) + "\\" + str(x))
